...
2. **[X] Phase 2: Backend Logic (n8n Workflows)**
   ...
   - **Retrieval & Chat Workflow:** Receive user query via Webhook → Generate Gemini Embedding for query → Search Qdrant for similar chunks → Send context + query to Gemini 1.5 → Return response to Frontend. *(Final Adjustments - Vector Store QA Tool)*
3. **[~] Phase 3: Frontend Development (Streamlit)**
   - Build a file uploader sidebar for PDF/text ingestion. *(Completed)*
   - Build a main chat interface for the user to ask questions. *(Pending API endpoints from n8n)*
   - Connect the UI to the n8n Webhooks via HTTP requests.
4. **[ ] Phase 4: Testing & Refinement**
   - End-to-end testing with sample documents.
   - Refine chunk sizes and prompt engineering.